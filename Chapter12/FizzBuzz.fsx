type FizzBuzzSequenceBuilder() =
  member __.Yield(v) =
    ...
    | _, 0 -> "Buzz"
    | _    -> v.ToString()
  member __.Delay(f) = f() |> Seq.singleton
  member __.Delay(f : unit -> string seq) = f()
  member __.Combine(l, r) =
    Seq.append (Seq.singleton l) (Seq.singleton r)
  member __.Combine(l, r) =
    Seq.append (Seq.singleton l) r
//  member x.Combine(l : string seq, r : string seq) =
//    Seq.append l r
  member __.For(g, f) = Seq.map f g
...
fizzbuzz { for x in 1..99 do yield x } |> Seq.take 10 |> Seq.toList;;
fizzbuzz { yield 1
           yield 2
