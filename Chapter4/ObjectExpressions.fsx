...
type Character(name : string, maxHP : int) =
  ...
  member __.Attack(o : Character) =
    let power = match __.Weapon with
                | Some(w) -> w.Power
                | None -> 1 // fists
    o.HP <- System.Math.Max(0, o.HP - power)
  override __.ToString() = sprintf "%s: %i/%i" name __.HP maxHP
...
let forgeWeapon desc power =
  { new IWeapon with
      member __.Description with get() = desc
      member __.Power with get() = power
    interface System.IDisposable with
      member __.Dispose() = printfn "Disposing" }
